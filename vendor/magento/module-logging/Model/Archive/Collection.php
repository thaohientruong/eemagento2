...
class Collection extends \Magento\Framework\Data\Collection\Filesystem
{
    ...
    protected function _generateRow($filename)
    {
        ...
        $date = new \DateTime();
        $date->createFromFormat('YmdH', str_replace('.csv', '', $row['basename']));
        $row['time'] = $date;
        /**
         * Used in date filter, becouse $date contains hours
         */
        $dateWithoutHours = new \DateTime();
        $dateWithoutHours->createFromFormat('YmdH', str_replace('.csv', '', $row['basename']));
