        ...
        _recountBundleRemQty: function () {
            ...
            collection.filter(':checked').each(function () {
                var itemId = $(this).data('args').item,
                    ...
                    itemReqQty = $('[id="items[' + itemIndex + '][' + bundleId + '][checkbox][qty][' + itemId + ']"]').val() * 1;
        ...
        _handleKeyup: function (e) {
            var currElem = $(e.currentTarget),
                ...
                if (currElemContainer.length < 1) {
                    currElemContainer = currElem.closest('fieldset').siblings('fieldset').find('select[id^="' + this.options.itemsItem + '"]');
                }
            var currElemIndex = currElemContainer.data('args').index;

            if (currElemId.match(/^items/) && !e.handled) {
                this._recountRemQty();

                var orderItemRemQty = 0,
