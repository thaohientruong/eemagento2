...
class TransactionDataBuilderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->randomMock = $this->getMockBuilder('Magento\Framework\Math\Random')
            ->disableOriginalConstructor()
            ->getMock();
        $this->localeResolverMock = $this->getMockBuilder('Magento\Framework\Locale\ResolverInterface')
            ->getMockForAbstractClass();
        $this->paymentInstanceMock = $this->getMockBuilder('Magento\Payment\Model\InfoInterface')
    ...
    public function buildExceptionDataProvider()
    {
        return [
            ...
            [
                'buildSubject' => [
                    'payment' => $this->getMockBuilder('Magento\Payment\Model\InfoInterface')->getMockForAbstractClass()
                ]
            ],
            [
