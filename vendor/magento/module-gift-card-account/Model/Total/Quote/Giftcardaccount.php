    ...
    public function collect(
        ...
        if ($baseUsed) {
            ...
            foreach ($quoteCards as $quoteCard) {
                $card = $quoteCard;
                if (
                    $quoteCard[ModelGiftcardaccount::BASE_AMOUNT] + $skipped <=
                    $quote->getBaseGiftCardsAmountUsed()
                ) {
                    $baseThisCardUsedAmount = $thisCardUsedAmount = 0;
                } elseif (
                    $quoteCard[ModelGiftcardaccount::BASE_AMOUNT] + $baseSaved >
                    $baseUsed
                ) {
                    ...
                    $quoteCard[ModelGiftcardaccount::BASE_AMOUNT] + $skipped + $baseSaved >
                    $quote->getBaseGiftCardsAmountUsed()
                ) {
                    $baseThisCardUsedAmount = min(
                        $quoteCard[ModelGiftcardaccount::BASE_AMOUNT],
