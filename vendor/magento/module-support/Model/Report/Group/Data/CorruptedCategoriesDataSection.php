...
class CorruptedCategoriesDataSection extends AbstractDataGroup
{
    ...
    {
        $data = [];
        $sql = 'SELECT `c`.`entity_id`, COUNT(`c2`.`children_count`) AS `children_count`,'
            . ' (LENGTH(`c`.`path`) - LENGTH(REPLACE(`c`.`path`,\'/\',\'\'))) AS `level`'
            . ' FROM `' . $tableName . '` AS `c`'
            . ' LEFT JOIN `' . $tableName . '` AS `c2` ON `c2`.`path` LIKE CONCAT(`c`.`path`,\'/%\')'
            . ' GROUP BY c.path';
        $expectedData = $this->connection->fetchAll($sql);
    ...
    {
        $data = [];
        $sql = 'SELECT `entity_id`, `children_count`, `level` FROM `' . $tableName . '`';
        $actualData = $this->connection->fetchAll($sql);
