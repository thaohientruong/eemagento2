<?php
namespace Ovi\Model\Api;
use Psr\Log\LoggerInterface;
/**
 * Class Instagram
 *
 * @package Ovi\Model\Api
 */
class Instagram
{
    /**
    ...
    public function __construct(
        ...
        if (is_array($config)) {
            ...
            $this->logger->critical('Error: __construct() - Configuration data is missing.');
        }
    }
    ...
    public function getLoginUrl($scopes = array('basic'))
    {
        ...
        $this->logger->critical("Error: getLoginUrl() - The parameter isn't an array or invalid scope permissions used.");
    }
    ...
    public function modifyRelationship($action, $user)
    {
        ...
        $this->logger->critical('Error: modifyRelationship() | This method requires an action command and the target user id.');
    }
    ...
    public function pagination($obj, $limit = 0)
    {
        ...
        $this->logger->critical("Error: pagination() | This method doesn't support pagination.");
    }
    ...
    protected function _makeCall($function, $params = null, $method = 'GET')
    {
        if (!isset($this->_accesstoken)) {
            $this->logger->critical("Error: _makeCall() | $function - This method requires an authenticated users access token.");
        }
        ...
        if (!$jsonData) {
            $this->logger->critical('Error: _makeCall() - cURL error: ' . curl_error($ch));
        }
    ...
    private function _makeOAuthCall($apiData)
    {
        ...
        if (!$jsonData) {
            $this->logger->critical('Error: _makeOAuthCall() - cURL error: ' . curl_error($ch));
        }
