...
/**
 * @see \Magento\Solr\SearchAdapter\Filter\Builder\Term
 */
class TermTest extends \PHPUnit_Framework_TestCase
{

    /**
     * @var \Magento\Solr\SearchAdapter\Filter\Builder\Term
     */
    private $target;
    ...
    protected function setUp()
    {
        ...
        $this->target = $objectManagerHelper->getObject(
            'Magento\Solr\SearchAdapter\Filter\Builder\Term',
            [
                'mapper' => $this->mapper
            ]
        );
    ...
    {
        $this->mapper->expects($this->any())->method('getFieldName')->with($field)
            ->willReturnCallback(
                function ($field) {
                    return 'attr_' . $field;
                }
    ...
    public function filterDataProvider()
    {
        return [
            ...
            [
                'field' => 'fieldName1',
                'value' => ['someValue', 2],
                'expected' => 'attr_fieldName1:"someValue" OR attr_fieldName1:"2"'
            ]
        ];
    }
    ...
    private function createRequestFilter($field, $value)
    {
        $requestFilter = $this->getMockBuilder('\Magento\Framework\Search\Request\Filter\Range')
            ...
            ->getMock();
        $requestFilter->expects($this->any())
            ->method('getField')
            ->willReturn($field);
        $requestFilter->expects($this->any())
            ->method('getValue')
