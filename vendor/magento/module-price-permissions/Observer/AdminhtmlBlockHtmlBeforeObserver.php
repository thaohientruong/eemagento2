...
class AdminhtmlBlockHtmlBeforeObserver implements ObserverInterface
{
    ...
    protected function _hidePriceElements($block)
    {
        ...
        if ($form !== null && $group !== null) {
            $fieldset = $form->getElement('group-fields-' . $group->getAttributeGroupCode());
        }

        if ($product !== null && $fieldset !== null) {
            $priceElementIds = [
                'special_price',
                'tier_price',
                'special_from_date',
            ...
            if (!$this->observerData->isCanEditProductPrice()) {
                ...
                if ($priceElement !== null && $typeId != \Magento\Catalog\Model\Product\Type::TYPE_BUNDLE
                ) {
                    if ($product->isObjectNew()) {
                        $priceElement->setValue($this->observerData->getDefaultProductPriceString());
                    }
                    $priceElement->setReadonly(true, false);
                    if (!$this->observerData->isCanReadProductPrice()) {
                        $priceElement->setType('hidden');
                        $priceElement->unsetData('after_element_html');
                    }
                }
                    ...
                    $amountsElement->setValue(
                        [
                            [
                                ...
                                'value' => $this->observerData->getDefaultProductPriceString(),
                                'website_value' => (double)$this->observerData->getDefaultProductPriceString()
                            ]
                        ]
                    );
                }
