            ...
            try {
                ...
                if ($isAddAction) {
                    ...
                    if ($model->setTypeById($typeId) === false) {
                        throw new LocalizedException(__('Incorrect Type'));
                    }
                }
                    ...
                    foreach ($registrantsPost as $registrant) {
                        if (is_array($registrant)) {
                            ...
                            if (!empty($registrant[$idField])) {
                                ...
                                if (!$person->getId()) {
                                    throw new LocalizedException(__('Please correct the registrant data.'));
                                }
                            } else {
                ...
                if (!$addressTypeOrId || $addressTypeOrId == \Magento\GiftRegistry\Helper\Data::ADDRESS_NEW) {
                    ...
                    if (!empty($data['address'])) {
                        ...
                        throw new LocalizedException(__('Address is empty.'));
                    }
                    if ($errors !== true) {
                        foreach ($errors as $err) {
                            $this->messageManager->addError($err);
                    ...
                    if (!$address) {
                        throw new LocalizedException(__('Please correct the address.'));
                    }
                    $model->importAddress($address);
                }
