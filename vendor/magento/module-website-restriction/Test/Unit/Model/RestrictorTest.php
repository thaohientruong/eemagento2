...
class RestrictorTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->config = $this->getMockBuilder('Magento\WebsiteRestriction\Model\ConfigInterface')
        ...
        $this->session = $this->getMockBuilder('Magento\Framework\Session\Generic')
            ->disableOriginalConstructor()
            ->setMethods(['setWebsiteRestrictionAfterLoginUrl'])
            ->getMock();

        $this->urlFactory = $this->getMockBuilder('Magento\Framework\UrlFactory')
            ->disableOriginalConstructor()
            ->setMethods(['create'])
            ->getMock();

        $this->model = $objectManager->getObject(
            'Magento\WebsiteRestriction\Model\Restrictor',
            [
                'config' => $this->config,
    ...
    public function testRestrictRedirectNot302Landing()
    {
        ...
        $urlMock = $this->getMockBuilder('Magento\Framework\Url')
            ->disableOriginalConstructor()
            ->setMethods(['getUrl'])
            ->getMock();
