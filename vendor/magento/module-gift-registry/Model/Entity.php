 ...
 * Entity data model
 *
 * @method \Magento\GiftRegistry\Model\ResourceModel\Entity getResource()
...
class Entity extends \Magento\Framework\Model\AbstractModel
{
    ...
    {
        $this->_init('Magento\GiftRegistry\Model\ResourceModel\Entity');
        parent::_construct();
    }
    ...
    public function exportAddressData()
    {
        $shippingAddressData = unserialize($this->getData('shipping_address'));
        if (is_array($shippingAddressData)) {
            $address = $this->addressDataFactory->create(['data' => $shippingAddressData]);
        } else {
                ...
                if (!isset($item['delete'])) {
                    ...
                    if ($stockItemDo->getIsQtyDecimal() == 0 && $item['qty'] != (int)$item['qty']) {
                        throw new \Magento\Framework\Exception\LocalizedException(
                            __('Please correct the  gift registry item quantity.')
                        );
                    }
                }
            } else {
