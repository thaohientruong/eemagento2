...
class ConfigProvider implements ConfigProviderInterface
{
    ...
    protected function getItemsInfo()
    {
        ...
        foreach ($this->getQuote()->getAllItems() as $quoteItem) {
            ...
            if (!$this->giftWrappingData->isGiftWrappingAvailableForProduct($product->getGiftWrappingAvailable())) {
                continue;
            }
            $gwPrice = [];
            ...
            if ($price) {
                ...
                if ($this->getDisplayWrappingBothPrices()) {
                    $gwPrice['price_incl_tax'] = $this->calculatePrice($item, $price, $shippingAddress, true);
                    $gwPrice['price_excl_tax'] = $this->calculatePrice($item, $price, $shippingAddress);
                } else {
                    $gwPrice['price'] = $this->calculatePrice(
                        $item,
                        $price,
                        $shippingAddress,
                        $this->getDisplayWrappingIncludeTaxPrice()
                ...
                $itemsData[$quoteItem->getId()] = $gwPrice;
            }
        }
        return $itemsData;
    }
    ...
    {
        return $this->giftWrappingData->isGiftWrappingAvailableForItems();
    }
    ...
    {
        return $this->giftWrappingData->allowPrintedCard();
    }
    ...
    {
        return $this->giftWrappingData->allowGiftReceipt();
    }
