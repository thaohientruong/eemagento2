...
class ConstraintProcessor
{
    ...
    public function resolve(
        ...
        if (!isset($this->strategies[$strategyCode])) {
            throw new LocalizedException(new Phrase('Unknown strategy code: %1', [$strategyCode]));
        }
    ...
    public function validate(ConstraintInterface $constraint, array $data)
    {
        ...
        if (empty($result)) {
            throw new LocalizedException(new Phrase('Cannot update row: a foreign key constraint fails'));
        }
    }
    ...
    protected function getInvolvedData(array $involvedData, $referenceField)
    {
        ...
        foreach ($involvedData as $item) {
            if (isset($item[$referenceField])) {
                ...
                throw new LocalizedException(new Phrase('Unknown field name: %1', [$referenceField]));
            }
        }
        return $output;
    }
