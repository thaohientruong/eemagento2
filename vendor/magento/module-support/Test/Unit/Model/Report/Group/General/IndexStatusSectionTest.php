...
class IndexStatusSectionTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    protected function setUp()
    {
        ...
        $this->indexerFactoryMock = $this->getMockBuilder('Magento\Indexer\Model\Indexer\CollectionFactory')
            ->disableOriginalConstructor()
            ->setMethods(['create'])
            ->getMock();
        $this->collectionFactory = $this->getMockBuilder('Magento\Indexer\Model\Indexer\Collection')
            ->disableOriginalConstructor()
            ->setMethods(['getItems'])
            ->getMock();

        $this->viewMock = $this->getMock('Magento\Framework\Mview\View', [], [], '', false);
        $this->timeZoneMock = $this->getMock('Magento\Framework\Stdlib\DateTime\TimezoneInterface');

        $this->categoryProductsIndexerMock = $this->getMockBuilder('Magento\Indexer\Model\Indexer')
            ->disableOriginalConstructor()
            ->setMethods(['getView', 'getTitle', 'getStatus', 'isValid', 'getLatestUpdated', 'getDescription'])
            ->getMock();
        $this->productCategoriesIndexerMock = $this->getMockBuilder('Magento\Indexer\Model\Indexer')
            ->disableOriginalConstructor()
            ->setMethods(['getView', 'getTitle', 'getStatus', 'isValid', 'getLatestUpdated', 'getDescription'])
            ->getMock();
        $this->catalogSearchIndexerMock = $this->getMockBuilder('Magento\Indexer\Model\Indexer')
            ->disableOriginalConstructor()
            ->setMethods(['getView', 'getTitle', 'getStatus', 'isValid', 'getLatestUpdated', 'getDescription'])
            ->getMock();

        $this->indexStatus = $this->objectManagerHelper->getObject(
            'Magento\Support\Model\Report\Group\General\IndexStatusSection',
            [
